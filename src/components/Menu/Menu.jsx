...
import { faPieChart, faCashRegister, faRegistered, faStoreAlt } from '@fortawesome/free-solid-svg-icons';
...
export default function Menu({visibility}) {
  ...
  return (
    ...
    <div className={`bg-white shadow-lg absolute py-6 translate-y-16 translate-x-0 rounded-md ${(!visibility) ? "hidden" : "block"}`}>
        <ul className='px-9 lg flex flex-col gap-4' >
             {
               data.map((item)=>(
                 <li key={item.id} className="grid grid-cols-1 justify-center justify-items-center p-4 gap-2 text-black">
